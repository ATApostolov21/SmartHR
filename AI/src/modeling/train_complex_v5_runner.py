...
MODEL_SAVE_PATH = 'models/neural_network/synthetic_model_v5.h5'
...
# Feature Selection - DROP FEWER FEATURES
# We deliberately want to keep weak signals now.
print("\n--- Feature Selection ---")
corr_matrix = df.corr()
...
# Only drop essentially zero correlation
...
def build_model(input_dim):
    model = Sequential([
        ...
        BatchNormalization(),
        Dropout(0.3),
        Dense(32, activation='relu'), # Added layer for complexity
        BatchNormalization(),
        Dropout(0.2),
...
plt.title('Confusion Matrix (Complex Data)')
...
print(f"Analysis saved to {REPORT_DIR}")
